...
logging.basicConfig(format=logging.BASIC_FORMAT, level=logging.INFO, handlers=[RichHandler()])
...
class Repl:
        ...
        while Repl.event_loop_active:
            with thread_mutex:
                for pdf in glob(os.path.join(self.pdf_loc, '*')):
                    ...
                    repl_logger.info("New candidate slip detected! : {}".format(os.path.basename(pdf)))
                    print('', end="")
    ...
    def start_repl(self):
        ...
        while Repl.event_loop_active:
            print(HTML("<green><b>type candidate name to get the diff OR 'help' for help. Use TAB for autocompletion</b></green>"))
            cmd: str = self.prompt_session.prompt(self.prompt_string, style=self.prompt_style, completer=self.word_completer)

            if cmd in self.candidate_names:
                pass
            elif cmd == "opdf":
                cmd_completion = subprocess.run(["explorer", self.pdf_loc], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
